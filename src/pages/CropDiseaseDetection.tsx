    ...
    <div className="min-h-screen bg-gradient-to-br from-background via-muted/20 to-accent/10 py-8 px-4">
            ...
            <CardContent className="p-6">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {userScans.slice(0, 8).map((scan) => (
                  <div key={scan.id} className="group p-6 bg-muted/30 rounded-xl border border-border hover:border-primary/50 transition-all duration-300 hover:shadow-lg hover:scale-105">
                    <div className="flex items-start justify-between mb-4">
                      <div className="flex items-center gap-3">
                        <div className={`w-4 h-4 rounded-full ${getResultColor(scan.result)}`} />
                        <span className="font-semibold text-base">{scan.crop}</span>
                      </div>
                    ...
                    <div className="space-y-3 mb-6">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-sm font-semibold">{scan.confidence}%</span>
                      </div>
              ...
              {userScans.length > 8 && (
                <div className="text-center mt-6">
                  <p className="text-sm text-muted-foreground">
                    Showing 8 of {userScans.length} scans
                  </p>
                </div>
              )}
            </CardContent>
      ...
      {showScanModal && selectedScan && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <Card className="max-w-4xl w-full max-h-[90vh] overflow-hidden shadow-lg border-0 bg-white">
            {/* Simple Green Header */}
            <CardHeader className="bg-green-600 text-white p-6">
              <div className="flex items-center justify-between">
                ...
                <Button
                  ...
                  className="h-10 w-10 rounded-full hover:bg-green-700 text-white p-0"
                >
                  <X className="h-5 w-5" />
                </Button>
              </div>
            ...
            <div className="overflow-y-auto max-h-[calc(90vh-120px)]">
              <CardContent className="p-8 space-y-6">
                ...
                <div className="bg-gray-50 rounded-xl p-6">
                  <h3 className="font-semibold text-gray-800 mb-4 text-lg">Analysis Results</h3>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="bg-white rounded-lg p-4 border border-gray-200">
                      <div className="text-xs text-gray-500 uppercase tracking-wide mb-2">Crop Type</div>
                      <div className="text-xl font-bold text-gray-800">{selectedScan.crop}</div>
                    </div>
                    
                    <div className="bg-white rounded-lg p-4 border border-gray-200">
                      <div className="text-xs text-gray-500 uppercase tracking-wide mb-2">Analysis Result</div>
                    ...
                    <div className="bg-white rounded-lg p-4 border border-gray-200">
                      ...
                      <div className="flex items-center gap-3">
                        <div className="text-xl font-bold text-blue-600">{selectedScan.confidence}%</div>
                ...
                <div className="bg-blue-50 rounded-xl p-6">
                  <h3 className="font-semibold text-blue-800 mb-4 text-lg">Export Options</h3>
                ...
                {selectedScan.diseaseDetails && (
                  <div className="space-y-6">
                    <h3 className="font-semibold text-gray-800 text-lg">Disease Information</h3>
                    
                    {/* Treatment */}
                    <div className="bg-red-50 rounded-xl p-6">
                      <h4 className="font-medium text-red-800 mb-3 text-lg">What to Do Now</h4>
                      <div className="space-y-2">
                        {selectedScan.diseaseDetails.treatment.slice(0, 3).map((treatment, index) => (
                          <div key={index} className="text-sm text-red-700 bg-white p-3 rounded-lg border border-red-200">
                            {index + 1}. {treatment}
                          </div>
                        ))}
                      </div>
                    ...
                    <div className="bg-green-50 rounded-xl p-6">
                      <h4 className="font-medium text-green-800 mb-3 text-lg">How to Prevent</h4>
                      <div className="space-y-2">
                        {selectedScan.diseaseDetails.prevention.slice(0, 3).map((prevention, index) => (
                          <div key={index} className="text-sm text-green-700 bg-white p-3 rounded-lg border border-green-200">
                            {index + 1}. {prevention}
                          </div>
                        ))}
                      </div>
                    ...
                    <div className="bg-orange-50 rounded-xl p-6">
                      <h4 className="font-medium text-orange-800 mb-3 text-lg">Warning Signs</h4>
                      <div className="space-y-2">
                    ...
                    {/* Quick Info */}
                    <div className="bg-gray-50 rounded-xl p-6">
                      <h4 className="font-medium text-gray-800 mb-3 text-lg">Disease Overview</h4>
                    ...
                    {/* Products */}
                    <div className="bg-green-50 rounded-xl p-6">
                      <h4 className="font-medium text-green-800 mb-3 text-lg">Recommended Products</h4>
                      <div className="flex flex-wrap gap-2">
                        {selectedScan.diseaseDetails.recommendedProducts.slice(0, 6).map((product, index) => (
                          <Badge key={index} variant="outline" className="text-sm p-2 bg-white border-green-200 text-green-700">
                            {product}
                          </Badge>
                        ))}
                      </div>
                ...
                {!selectedScan.diseaseDetails && (
                  <div className="bg-green-50 rounded-xl p-6 text-center">
                    ...
                    <h4 className="font-semibold text-green-800 mb-3 text-lg">Scan Completed Successfully!</h4>
